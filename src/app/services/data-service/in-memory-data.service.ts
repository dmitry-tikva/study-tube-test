...
import { UsersMock } from '@assets/mock/mock-users';
import { LearningsMock } from '@assets/mock/mock-learnings';
...
export class InMemoryDataService implements InMemoryDbService {
  createDb() {
    ...
    const learnings = LearningsMock;

    return { users, learnings };
  }
  ...
  private static applyQuery(
    ...
    return collection.filter((row: any) => {
      if (row.name && row.name.indexOf(query.get('search')) > -1) {
        return true;
      }
      if (row.title && row.title.indexOf(query.get('search')) > -1) {
        return true;
      }
      if (row.firstName && row.firstName.indexOf(query.get('search')) > -1) {
        return true;
      }
      if (row.lastName && row.lastName.indexOf(query.get('search')) > -1) {
        return true;
      }
      if (row.email && row.email.indexOf(query.get('search')) > -1) {
        return true;
      }
